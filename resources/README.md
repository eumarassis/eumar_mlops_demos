...
The databricks CLI bundles top level is defined by file `eumar_mlops_demos/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`eumar_mlops_demos/resources/model-workflow-resource.yml`)
 - batch inference workflow (`eumar_mlops_demos/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`eumar_mlops_demos/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`eumar_mlops_demos/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`eumar_mlops_demos/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `eumar_mlops_demos/databricks.yml` and `eumar_mlops_demos/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-eumar_mlops_demos-model     | /dev-eumar_mlops_demos-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-eumar_mlops_demos-model | /staging-eumar_mlops_demos-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-eumar_mlops_demos-model    | /prod-eumar_mlops_demos-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-eumar_mlops_demos-model    | /test-eumar_mlops_demos-experiment    |
...
2. Complete the `TODO` in `eumar_mlops_demos/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `eumar_mlops_demos/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `eumar_mlops_demos/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-eumar_mlops_demos-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-eumar_mlops_demos-batch-inference-job`
that runs the notebook under `eumar_mlops_demos/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-eumar_mlops_demos-batch-inference-job`.
...
`test-eumar_mlops_demos-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-eumar_mlops_demos-batch-inference-job`.
